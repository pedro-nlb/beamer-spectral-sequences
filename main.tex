...
\begin{frame}
    ...
    A filtration on the complex $F^{\bullet}C$ induces a filtration in cohomology
...
\begin{frame}
	...
	\begin{column}{0.9\textwidth}
	    \begin{itemize}
		\item Some $c\in C^{0}\setminus F^{1}C^{0}$ may end up in $F^{1}C^{1}$ under $d^{0}$.
		    \pause
		\item The $\gr^{p}C$'s will not be able to detect this.
		    \pause
		\item Don't panic: \usebeamercolor[fg]{frametitle}{spectral sequences will account for that}.
	    \end{itemize}
	\end{column}
    \end{columns}
\end{frame}
...
\begin{frame}
    \frametitle{\textsc{Baby Example}: turn the page by taking cohomology again}
    The ``$E_{2}$-page'' reads:
    \begin{tikzpicture}[ampersand replacement=\&]
	\matrix (m) [matrix of math nodes,
	nodes in empty cells, nodes={minimum width=5ex,
    ...
    \begin{itemize}
	\item If we had started from a longer complex, the original differentials would induce arrows of type $(2,-1)$.
    \end{itemize}
\end{frame}
...
\begin{frame}
	...
	column sep=7ex, row sep=1ex]{
	    \&  \&  \& \\
	    \& \frac{C^{1}}{F^{1}C^{1}+\im(d^{0})} \& 0 \& \\
	    \& \frac{\ker(d^{0})}{\ker\left(d^{0}|_{F^{1}C}\right)} \& \frac{F^{1}C^{1}+\im(d^{0})}{\im(d^{0})} \& \\
	    \hspace{2mm} \& 0 \& \ker\left(d^{0}|_{F^{1}C}\right) \& \\
	    \quad\strut \& \& \& \strut \\};
	\draw[blue,-stealth,dashed,shorten <=5mm,shorten >=5mm] (m-1-1) node{\usebeamercolor[fg]{frametitle}{$p+q=1$}} -- (m-4-4) node{\usebeamercolor[fg]{frametitle}{$H^{1}$}};
	\draw[blue,-stealth,dashed,shorten <=5mm,shorten >=5mm] (m-2-1) node{\usebeamercolor[fg]{frametitle}{$p+q=0$}}-- (m-5-4) node{\usebeamercolor[fg]{frametitle}{$H^{0}$}};
    \end{tikzpicture}
\end{frame}
...
\begin{frame}
    ...
    \begin{center}
	\usebeamercolor[fg]{frametitle}{``Elements in $F^{p}C^{p+q}$ which will fall down at least $r$ levels

	and we don't care what happens in $F^{p+1}C^{p+q}$.''}
    \end{center}
    \pause
    ...
    \begin{center}
	\usebeamercolor[fg]{frametitle}{``Elements in $F^{p}C^{p+q}$ which came falling down at most $r-1$ levels

	and we don't care about the ones in $F^{p+1}C^{p+q}$.''}
    \end{center}
\end{frame}
